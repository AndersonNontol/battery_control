...
void app_main(void)
{
	...
	printf("This is the first line printed");
	// Let's see why the device is asleep
	esp_sleep_wakeup_cause_t cause = esp_sleep_get_wakeup_cause();
	if(cause == ESP_SLEEP_WAKEUP_UNDEFINED){ // If the device is not asleep after a wake up
		...
		printf("The device woke up because of the ULP\n");
	}
	
	else if (cause == ESP_SLEEP_WAKEUP_TIMER) {
		printf("The device woke up because of the timer\n");
		printf("The example variable is:%"PRIu32"\n", ulp_example_variable);
	}
	
	else{ // The device is asleep after a wake up from ulp
		printf("Woken up by the TIMER");
	}
